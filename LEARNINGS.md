...
- 2026-05-26 #75: towerIds computed inline from Object.values(TOWER_DEFS).map each render creates a new array reference on every render, causing the hotkey useEffect to re-register needlessly — useMemo or hoisting to module scope would be the zero-cost fix, worth noting in AGENTS.md as a pattern for stable dep arrays.
- 2026-05-26 #108: Extracting a Shell component inside BrowserRouter (to use useLocation for route-conditional layout) is the correct pattern for per-route layout variants in React Router v6 — plan for it upfront whenever any route needs a structurally different wrapper than the global default.
- 2026-05-26 #109: Source-position tests (indexOf ordering) are an effective complement to source-read containment tests — together they assert both presence and structural placement without a render harness, making them a reliable pair for layout-conditional JSX changes.
